...
- 차이점: (차이점이 없으면 `없음`으로 적고, 차이점이 있으면 각 항목을 줄바꿈으로 구분하되 `[차이점 1]` 같은 소제목과 `(A)`, `(B)` 같은 임시 구성 라벨은 쓰지 않는다. 각 항목은 차이가 나는 청구항 제한 문언으로 바로 시작한다. 대응 구절은 한국어 문헌이면 원문 그대로 적고 출처를 괄호로 병기하며, 외국어(일본어·중국어·영어 등) 문헌이면 1줄에 한국어 직역 또는 준직역문을 적고 다음 줄 괄호 안에 단락/본문 위치와 원문 발췌를 함께 적는다)
...
- 차이점: (각 항목은 줄바꿈으로 구분하되 `[차이점 1]`, `[차이점 2]` 같은 소제목과 `(A)`, `(B)` 같은 임시 구성 라벨은 쓰지 않는다. 각 항목은 차이가 나는 청구항 제한 문언으로 바로 시작하고, 인용발명 1에 없는 부분이 인용발명 2의 어느 구성에 대응되는지, 그 구성의 대응 구절과 출처를 함께 적고, 왜 대응되는지와 결합 판단을 간결히 정리한다. 한국어 문헌은 원문 그대로 적고 출처를 괄호로 병기하며, 외국어(일본어·중국어·영어 등) 문헌은 1줄에 한국어 직역 또는 준직역문을 적고 다음 줄 괄호 안에 단락/본문 위치와 원문 발췌를 함께 적는다. 주지관용기술은 문헌 근거가 없는 차이점에만 최후 수단으로 사용. 독립항 결합형에서 `주지관용 구성 입증자료`로 표시된 예외적 제3문헌만 표시된 일반 구성의 입증자료로 제한한다)
